...
3. When the host supplies the current native Codex session ID, build the private argument pair `--exclude-current --current-native-id <native-id>` and append it to every `tang browse --json` or `tang search <query> --json` call in this workflow. Tang resolves this against the indexed project database, so it does not trigger another native scan. This exact exclusion prevents the active session and its just-recorded search request from appearing as a source. An unindexed current session cannot be returned and needs no exclusion. If host evidence is absent, run browse or search **without** `--exclude-current` and disclose before presenting results: **“Tang cannot identify the active session from this host, so it may appear in these results.”** Do not infer which result is active from recency, rank, title, UUID, or prose; if the user recognizes the active session in the displayed results, do not select it as a source. Continue with the user's explicit source selection. Self-link rejection and exact-target confirmation still protect continuation recording. Run `tang browse --json --page 1` for an overview or pass the user's remembered keywords or quoted phrase as the single query argument to `tang search <query> --json --page 1`. Do not broaden discovery outside the current project. If a search returns no results, ask for a different phrase instead of inventing a candidate.
...
6. Run `tang context <source-id>... --json` with exactly the selected IDs from the private map. If every source fails, explain the error and return to selection. If only some fail, disclose the warnings and continue with the cited evidence that remains. When the pack JSON shows `status: complete` but includes `constraint_signals` or a deterministic first-goal conflict warning, treat that as **successful reads with disagreeing constraints**, not permission to skip conflict disclosure or user clarification.
...
When the host supplies the active native session ID, this host workflow can record selected predecessors **only into that active session**. Do not accept a historical `--to` handle in place of the active target. To connect two already indexed historical handles, leave the host workflow and use the normal-terminal command `tang link --from <source-handle> --to <target-handle>` only after reviewing source context and explicitly confirming that exact edge set.
...
If the user asks to reopen an indexed Codex handle itself, explain that `tang resume <handle>` is a normal-terminal launcher and must not be run as a nested interactive process inside this Codex session. It reopens only that native session; it does not replace this source-cited recovery workflow or create a continuation link.
