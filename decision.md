...
## 2026-07-22T00:00:00Z · v0.3.0 · Promote four-harness continuity
- Context: The hackathon submission is closed, Epic 10 already provides read-only Codex, Grok, OpenCode, and Cursor adapters plus explicit Tang-owned link destinations, and the human requested a v0.3.0 release in which all four can be read, linked, and resumed with Tang. Installed Grok Build 0.2.106 and Cursor Agent 2026.07.17 expose exact native `--resume` contracts; local stores use UUID session identities. Native transcript mutation remains forbidden.
- Options: (a) publish only the existing beta read/link work and keep resume limited to Codex/OpenCode; (b) describe manual context handoff as resume; (c) add guarded native launch adapters for Grok and Cursor, preserve `.tang`-only graph writes, and publish one precise four-harness capability matrix.
