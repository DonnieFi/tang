...
Use `tang browse --json` or `tang search QUERY --json`. Present only the returned handles, redacted fields, and current page choices. Keep canonical IDs private. Ask the user to select exact displayed choices; never infer a source from prose, recency, or a title.
