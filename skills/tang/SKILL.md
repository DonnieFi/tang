...
5. After a successful JSON result, verify `source_ids` exactly match the selection and read `target_id` from that result. A result with `inserted: 0` and positive `existing` is an idempotent replay of already-confirmed edges, not permission to add or infer anything else. Run `tang graph <target_id>` with the same project and database options, then present that shared-renderer Multiverse Map as the final reveal. Do not invent, annotate, or persist additional edges.
